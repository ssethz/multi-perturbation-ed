...
def get_bs_dags(num_bs, obs_samples, nsamples_obs, nnodes, cheat_cpdag=None, bic=True):
    ...
    cheat_dag is for debugging: on the first round forces the cpdag given into the sample
    bic: whether to have your prior given by the bayesian information criterion
    """
...
def MI_obj_gen(M, bs_dags, iv_means, K=50, iv_var = 0.01):
    ...
    K is number of samples obtained by each intervention
...
def union_cpdag(cpdags):
    ...
    #for very large sets of bootstrap dags should condense the list to unique cpdags
    for cpdag in cpdags:
...
def gred_infinite(bs_dags, n_b, k, obj):
    ...
    n = (bs_dags[0]['dag']).shape[0]
    #just input the first cpdag- its not used in this case
...
def process_score(inter, bs_dags, loss, OVs, meth, f, k, b, k_range, inters_dict):
    """
    save objective value and interventions, while considering a special case for when 
    ss a is used 
    """
    obj_val = loss(inter)
...
def run_finite_experiment(nnodes, generator, k_range, meths, labs, title = '', name='', repeats=10, nsamples_obs=1000, K=20, M=20, num_bs=10):
    """
    ...
    for _ in range(repeats):
        ...
        while valid_dag == False:
            ...
            #accept dags with mec size less than 100 and more than nnodes
            #print(max_score)
...
if __name__ == '__main__':
    ...
    for nnodes in [20,40]:
        for i in range(len(repeat_list)):
            ...
            run_finite_experiment(nnodes, generator, k_range, meths, labs, title = title, name=name, repeats=repeat_list[i], nsamples_obs=nnodes*50, K=3, M=10, num_bs=100)
